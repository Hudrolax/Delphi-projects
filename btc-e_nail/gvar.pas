...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        Result := 'Perestavlay Trail. Para ' + VirtualOrder.pair + ', tip ' + VirtualOrder.OType + ', cena ' + FloatToStr(VNewRate)
        + ', otstup ' + FloatToStr(VirtualOrder.delta) + ', shag ' + FloatToStr(temp_step)
        + ', verhnaya cena ' + FloatToStr(Arr[0].price) + ', obiem sverhy ' + FloatToStr(VAmount);
      end;
    end;
    ...
    begin
      ...
      if (VirtualOrder.Next_Chance>=80) then // Сразу не продаемся, - вдруг свеча, даем рынку "Next_Chance" раз шанс одуматься и восстановить цену. Nail

      begin
        ComplateRate := VirtualOrder.rate;
        if Arr[Length(Arr) - 1].price < ComplateRate then
          ComplateRate := Arr[Length(Arr) - 1].price;
        SendOrder(VirtualOrder.pair, VirtualOrder.OType, ComplateRate, VirtualOrder.amount, true);     // внимание
        Result := 'Ispolnen order po Trailing Stop (' + VirtualOrder.pair + ', ' + VirtualOrder.OType + ')! Cena >= ' + FloatToStr(VirtualOrder.rate)
          + ', summa lota ' + FloatToStr(VirtualOrder.amount) + ', otstup ' + FloatToStr(VirtualOrder.delta)+ ', shag ' + FloatToStr(VirtualOrder.step)
          + ', obiem nad orderom bil ' + FloatToStr(VAmount)+ ', jdali raz ' + IntToStr(VirtualOrder.Next_Chance);
        VirtualOrder.Next_Chance:=0;
        ...
        begin
          ...
          VirtualOrders[LengthVO].rate := VirtualOrder.rate*0.995;     // внимание
          VirtualOrders[LengthVO].min_rate := VirtualOrder.rate*0.995;
          ...
          Result:= Result +' Cozdal protivopolojniy order';
        end;

        VirtualOrder.deleted := true;
      end
      else inc(VirtualOrder.Next_Chance);  //Nail
    end;
  ...
  begin
    ...
    begin
      ...
      begin
        VirtualOrder.rate := VNewRate;
        Result := 'Perestavliay Trail. Para ' + VirtualOrder.pair + ', tip ' + VirtualOrder.OType + ', cena ' + FloatToStr(VNewRate)
        + ', otstup ' + FloatToStr(VirtualOrder.delta) + ', shag ' + FloatToStr(temp_step)
        + ', verhniya cena ' + FloatToStr(Arr[0].price) + ', obiem sverhy ' + FloatToStr(VAmount);
      end;
    end;
    ...
    begin
      ...
      if (VAmount <= (VirtualOrder.amount + AvergeAmount) * VirtualOrder.MultiAverge) or (Arr[0].price >= VirtualOrder.rate) then     //внимание
      if (VirtualOrder.Next_Chance>=80) then // Сразу не продаемся, - вдруг свеча, даем рынку "Next_Chance" раз шанс одуматься и восстановить цену. Nail
      // Если над целевой ценой осталось объема менее объема ордера+% - нахуй
      begin
        ComplateRate := VirtualOrder.rate;
        ...
        begin
          ...
          VirtualOrders[LengthVO].pair := VirtualOrder.pair;
          VirtualOrders[LengthVO].OType := ReverseOrderType(VirtualOrder.OType);
          VirtualOrders[LengthVO].amount := VirtualOrder.amount*0.998;                   //внимание
          if VirtualOrders[LengthVO].amount<0.1 then VirtualOrders[LengthVO].amount:=0.1;
          VirtualOrders[LengthVO].rate := VirtualOrder.rate*1.006;
          VirtualOrders[LengthVO].min_rate := VirtualOrder.rate*1.006;
          VirtualOrders[LengthVO].step := VirtualOrder.step2;
          VirtualOrders[LengthVO].first_step := VirtualOrder.first_step;
          VirtualOrders[LengthVO].MultiAverge := VirtualOrder.MultiAverge;
          VirtualOrders[LengthVO].delta := VirtualOrder.delta2;
          ...
          Result:= Result + ' Sozdal protivopolojniy order.';
        end;
