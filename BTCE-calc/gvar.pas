...
interface
uses SysUtils, Types, Windows, TLHELP32, Classes, StdCtrls;
...
var
Ex1Ask,Ex1Bid,Ex2Ask,Ex2Bid:array of TOrder;
...
if k=length(s) then IsDigit:=true else IsDigit:=false;
end;
...
begin
  ...
  for i:=1 to Length(s) do
   begin
    if Copy(s,i,1) = ',' then zpt:=True;
    result:=result+Copy(s,i,1);
    if zpt then k:=k+1;
    if k>=amt+1 then Break;
   end;
end;
