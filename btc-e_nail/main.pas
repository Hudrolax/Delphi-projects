...
uses
  ...
  VCLTee.TeeProcs, VCLTee.Chart, inifiles, VCLTee.TeeGDIPlus, TLHelp32;
...
type
  TForm1 = class(TForm)
    ...
    BTC_Proxy_RunAndKill: TTimer;
    NextChanceVol: TEdit;
    Label58: TLabel;
    TabSheet21: TTabSheet;
    cbStrategCT: TCheckBox;
    Edit1: TEdit;
    Button9: TButton;
    Label59: TLabel;
    EditProcentCT: TEdit;
    ...
    Label64: TLabel;
    EditVolToCT: TEdit;
    EditProcentCTbuy: TEdit;
    EditVolumeCTbuy: TEdit;
    EditVolToCTbuy: TEdit;
    Label61: TLabel;
    Ask_LBL: TLabel;
    Bids_LBL: TLabel;
    Ask_LBL2: TLabel;
    ...
    procedure BTC_Proxy_RunAndKillTimer(Sender: TObject);
    procedure AskTimer1Timer(Sender: TObject);
    procedure AskTimer2Timer(Sender: TObject);
    procedure AskTimer3Timer(Sender: TObject);
    procedure BidsTimer1Timer(Sender: TObject);
    procedure BidsTimer2Timer(Sender: TObject);
    procedure BidsTimer3Timer(Sender: TObject);
  private
...
var
  price, volCTAsk, volCTBids, AskPrice, AskPrice2, AskPrice3, PrevAskPrice, BidsPrice, BidsPrice2, BidsPrice3, LastBuy, LastSell: real;
  i,j, k: Integer;
  ColorBar: TColor;
  PlacedOrder: string;
  FoundedSameOrder, FoundedOneOrder, foundedCT, foundedCT2, foundedCT3: boolean;
begin
 try
 LastBuy:=0;
 LastSell:=0;
 for i := (Length(OrderHistory)-1) DownTo 0 do
  begin
  if OrderHistory[i].pair = 'ltc_usd' then
     begin
       if OrderHistory[i].OType = 'sell' then LastSell:= OrderHistory[i].rate;         // запоминаем стоимость последних сделок, что бы торговать в плюс
       if OrderHistory[i].OType = 'buy' then LastBuy:= OrderHistory[i].rate;           // что бы не усложнять алгоритм поиска тупо проходимся с конца списка в начало
     end;
  end;
 Form1.ListBox2.items.Clear;          // получаем инфу о стенках и объемах стакана продаж
 if IsDigit(Form1.EditVolumeCT.Text) then
  begin
   volCTAsk:=0;
   AskPrice:=0;
   BidsPrice:=0;
   ...
   if Length(AsksLTCUSD) > 0 then
    for I := 0 to (Length(AsksLTCUSD)-1) do
     begin
      volCTAsk:=volCTAsk+ AsksLTCUSD[i].Volume;
      if (AsksLTCUSD[i].Volume>= StrToFloat(Form1.EditVolumeCT.Text)) then
       begin
        //
        if (StrToFloat(Form1.EditVolumeCT.Text)>=AsksLTCUSD[i].price) then
         begin
           if foundedCT2 then
            begin // нашли нашу 3 предстеночную цену продажи
             AskPrice3:=AsksLTCUSD[i].price;
            end;
           if foundedCT and not foundedCT2 then
            begin // нашли нашу 2 предстеночную цену продажи
             AskPrice2:=AsksLTCUSD[i].price;
             foundedCT2:=true;
            end;
           if not foundedCT then
            begin
              AskPrice:=AsksLTCUSD[i].price; // нашли нашу 1 предстеночную цену
              if (AskPrice-LastBuy)>=(AskPrice*0.008) then //проверяем, не вышли ли за пределы прибыльности с условием двойной комсы
               begin
                foundedCT:=true;
               end else // проверяем не вышли ли за объем предполагаемой свечки в поле Form1.EditVolToCT.Text, если вышли - ставим цену перед этим ордером
               begin
                 if (volCTAsk>=StrToFloat(Form1.EditVolToCT.Text)) then foundedCT:=true;
               end;
            end;
          end;
        // пишем в окно список стенок и объемов для визуального контроля стаканов
        Form1.ListBox2.items.Add(FloatToStr(AsksLTCUSD[i].Volume) +'$ по '+FloatToStr(AsksLTCUSD[i].price)+' и '+FloatToStr(volCTAsk)+' объем до ордера');
       end;
     end;
   Form1.Ask_LBL.Caption:= FloatToStr(AskPrice);    // показываем наши 3 уровня стенов
   Form1.Ask_LBL2.Caption:= FloatToStr(AskPrice2);
   Form1.Ask_LBL3.Caption:= FloatToStr(AskPrice3);
   if AskPrice2=0 then AskPrice2:=AskPrice+0.2;   // если стенка пустая (не хватило инфы глубины стаканов) - тупо ставим чуть выше
   if AskPrice3=0 then AskPrice3:=AskPrice2+0.2;

   Form1.ListBox3.items.Clear;    // получаем инфу о стенках и объемах стакана покупок
   if IsDigit(Form1.EditVolumeCTbuy.Text) then
    begin
     volCTBids:=0;
     BidsPrice:=0;
     foundedCT:=false;
     foundedCT2:=false;
     foundedCT3:=false;
     if Length(BidsLTCUSD) > 0 then
      for I := 0 to (Length(BidsLTCUSD)-1) do
       begin
        volCTBids:=volCTBids+BidsLTCUSD[i].Volume;
        if ((BidsLTCUSD[i].Volume)>= StrToFloat(Form1.EditVolumeCTbuy.Text)) then
         begin
          if (StrToFloat(Form1.EditVolumeCT.Text)>=AsksLTCUSD[i].price) then
         begin
           if foundedCT2 then
           begin // нашли нашу 3 предстеночную цену
             BidsPrice3:=BidsLTCUSD[i].price;
           end;
           if foundedCT and not foundedCT2 then
           begin // нашли нашу 2 предстеночную цену
            BidsPrice2:=BidsLTCUSD[i].price;
            foundedCT2:=true;
           end;

          if (StrToFloat(Form1.EditVolumeCTbuy.Text)>= BidsLTCUSD[i].price) and not foundedCT then
           begin
            BidsPrice:=BidsLTCUSD[i].price; // нашли нашу 1 предстеночную цену
            if (LastSell-BidsPrice)>=(BidsPrice*0.008) then
             begin
              foundedCT:=true;   // Проверяем, не вылезли ли за пределы двойной комсы
             end else  // проверяем не вышли ли за объем предполагаемой свечки в поле Form1.EditVolToCTbuy.Text, если вышли - ставим цену перед этим ордером
              begin
                if (volCTBids>=StrToFloat(Form1.EditVolToCTbuy.Text)) then foundedCT:=true;
              end;
           end;
          Form1.ListBox3.items.Add(FloatToStr(BidsLTCUSD[i].Volume) +'$ по '+FloatToStr(BidsLTCUSD[i].price)+' и '+FloatToStr(volCTBids)+' объем до ордера');
         end;
       end;
    end;
  end;
  Form1.Bids_LBL.Caption:= FloatToStr(BidsPrice);     // показываем наши 3 уровня стенов
  Form1.Bids_LBL2.Caption:= FloatToStr(BidsPrice2);
  Form1.Bids_LBL3.Caption:= FloatToStr(BidsPrice3);
  if BidsPrice2=0 then BidsPrice2:=BidsPrice-0.2;    // если стенка пустая (не хватило инфы глубины стаканов) - тупо ставим чуть выше
  if BidsPrice3=0 then BidsPrice3:=BidsPrice2-0.2;

  end;
   if Form1.cbStrategCT.checked then    // выставляем пристеночные ордера продаж стенки 1
    if IsDigit(Form1.EditVolumeCT.Text) and IsDigit(Form1.EditProcentCt.Text) then
      begin
        FoundedOneOrder:=false;
        for k := 0 to Length(ActiveOrders) - 1 do
           if ActiveOrders[k].pair = 'ltc_usd' then
            if ActiveOrders[k].OType = 'sell' then
            if (ActiveOrders[k].amount=StrToFloat(Form1.EditProcentCT.Text)) then
             begin
                  begin
                   if not ((ActiveOrders[k].rate = (AskPrice-0.0005))) then  //отменяем старые ордера с таким же объемом, но другой ценой
                     if Form1.AskTimer1.Tag=0 then
                     begin
                      CancelOrder(ActiveOrders[k].order_id);
                     end;
                  end;
             end;

        Form1.EditPriceOpen.Text:= FloatToStr(AskPrice-0.0005);
        Form1.EditAmountOpen.Text:= Form1.EditProcentCT.Text;
        FoundedSameOrder:=false;

         for k := 0 to Length(ActiveOrders) - 1 do      // ищем ордер для стенки 1
          if ActiveOrders[k].pair = 'ltc_usd' then
            if ActiveOrders[k].OType = 'sell' then
             begin
              if ((AskPrice-0.0005) = (ActiveOrders[k].rate)) then
               begin
                FoundedSameOrder:=true;
               end;
             end;

        if not FoundedSameOrder then    //ставим ордер на стенку 1 если не обнаружен точно такой же ранее выставленный ордер
        if Balance.LTC>=(1.003*StrToFloat(Form1.EditProcentCT.Text)) then
        if Form1.AskTimer1.Tag=0 then
          begin
           SendOrder('ltc_usd', 'sell', AskPrice-0.0005, StrToFloat(Form1.EditProcentCT.Text), true);
           Form1.AskTimer1.Enabled:=True;     // ставим таймер на 3 секунды, что бы не выставить в цикле несколько ордеров подряд из-за позднего ответа биржи
           Form1.AskTimer1.Tag:=1;
          end;

         // ----------  ищем отмены ордера для стенки 2
         FoundedOneOrder:=false;
        for k := 0 to Length(ActiveOrders) - 1 do
           if ActiveOrders[k].pair = 'ltc_usd' then
            if ActiveOrders[k].OType = 'sell' then
            if (ActiveOrders[k].amount=2*StrToFloat(Form1.EditProcentCT.Text)) then
             begin
                  begin
                   if not ((ActiveOrders[k].rate = (AskPrice2-0.0005))) then  //отменяем старые ордера с таким же объемом, но другой ценой
                    if Form1.AskTimer2.Tag=0 then
                     begin
                      CancelOrder(ActiveOrders[k].order_id);
                     end;
                  end;
         ...
         FoundedSameOrder:= false;
         for k := 0 to Length(ActiveOrders) - 1 do      // ищем ордер для стенки 2
          if ActiveOrders[k].pair = 'ltc_usd' then
            if ActiveOrders[k].OType = 'sell' then
             begin
              if ((AskPrice2-0.0005) = (ActiveOrders[k].rate)) then
               begin
                FoundedSameOrder:=true;
               end;
             end;

        if not FoundedSameOrder then    //ставим ордер на стенку 2 если не обнаружен точно такой же ранее выставленный ордер
        if Balance.LTC>=2*(1.003*StrToFloat(Form1.EditProcentCT.Text)) then
        if Form1.AskTimer2.Tag=0 then
         begin
           SendOrder('ltc_usd', 'sell', AskPrice2-0.0005, 2*StrToFloat(Form1.EditProcentCT.Text), true);
           Form1.AskTimer2.Enabled:=True;   // ставим таймер на 3 секунды, что бы не выставить в цикле несколько ордеров подряд из-за позднего ответа биржи
           Form1.AskTimer2.Tag:=1;
         end;

        // ----------      ищем отмены ордера для стенки 3
        FoundedOneOrder:=false;
        for k := 0 to Length(ActiveOrders) - 1 do
           if ActiveOrders[k].pair = 'ltc_usd' then
            if ActiveOrders[k].OType = 'sell' then
            if (ActiveOrders[k].amount=4*StrToFloat(Form1.EditProcentCT.Text)) then
             begin
                  begin
                   if not ((ActiveOrders[k].rate = (AskPrice3-0.0005))) then  //отменяем старые ордера с таким же объемом, но другой ценой
                    if Form1.AskTimer3.Tag=0 then
                     begin
                      CancelOrder(ActiveOrders[k].order_id);
                     end;
                  end;
         ...
         FoundedSameOrder:= false;
         for k := 0 to Length(ActiveOrders) - 1 do      // ищем существующий ордер для стенки 3
          if ActiveOrders[k].pair = 'ltc_usd' then
            if ActiveOrders[k].OType = 'sell' then
             begin
              if ((AskPrice3-0.0005) = (ActiveOrders[k].rate)) then
               begin
                FoundedSameOrder:=true;
               end;
             end;

        if not FoundedSameOrder then    //ставим ордер на стенку 3 если не обнаружен точно такой же ранее выставленный ордер
        if Balance.LTC>=4*(1.003*StrToFloat(Form1.EditProcentCT.Text)) then
        if Form1.AskTimer3.Tag=0 then
         begin
           SendOrder('ltc_usd', 'sell', AskPrice3-0.0005, 4*StrToFloat(Form1.EditProcentCT.Text), true);
           Form1.AskTimer3.Enabled:=True;  // ставим таймер на 3 секунды, что бы не выставить в цикле несколько ордеров подряд из-за позднего ответа биржи
           Form1.AskTimer3.Tag:=1;
         end;

      end;

    // покупки
    if Form1.cbStrategCT.checked then  //выставляем пристеночные ордера покупок
    if IsDigit(Form1.EditVolumeCTbuy.Text) and IsDigit(Form1.EditProcentCtbuy.Text) then
      begin
         // ----------     ищем отмены ордера для стенки 1
        FoundedOneOrder:=false;
        for k := 0 to Length(ActiveOrders) - 1 do
           if ActiveOrders[k].pair = 'ltc_usd' then
            if ActiveOrders[k].OType = 'buy' then
            if (ActiveOrders[k].amount=StrToFloat(Form1.EditProcentCTbuy.Text)) then
             begin
                  begin
                   if not ((ActiveOrders[k].rate = (BidsPrice+0.0005)) ) then  //отменяем старые ордера с таким же объемом, но другой ценой
                    if Form1.BidsTimer1.Tag=0 then
                     begin
                      CancelOrder(ActiveOrders[k].order_id);
                     end;
                  end;
         ...
         FoundedSameOrder:= false;
         for k := 0 to Length(ActiveOrders) - 1 do      // ищем существующий ордер для стенки 1
          if ActiveOrders[k].pair = 'ltc_usd' then
            if ActiveOrders[k].OType = 'buy' then
             begin
              if ((BidsPrice+0.0005) = (ActiveOrders[k].rate)) then
               begin
                FoundedSameOrder:=true;
               end;
             end;
        ...
        if not FoundedSameOrder then    //ставим ордер на стенку 1 если не обнаружен точно такой же ранее выставленный ордер
        if (Balance.USD/(BidsPrice+0.0005))>=(StrToFloat(Form1.EditProcentCTbuy.Text)*1.003) then
        if Form1.BidsTimer1.Tag=0 then
         begin
           SendOrder('ltc_usd', 'buy', BidsPrice+0.0005, StrToFloat(Form1.EditProcentCTbuy.Text), true);
           Form1.BidsTimer1.Enabled:=True;  // ставим таймер на 3 секунды, что бы не выставить в цикле несколько ордеров подряд из-за позднего ответа биржи
           Form1.BidsTimer1.Tag:=1;
         end;
         //--------
        ...
        for k := 0 to Length(ActiveOrders) - 1 do
           if ActiveOrders[k].pair = 'ltc_usd' then
            if ActiveOrders[k].OType = 'buy' then
            if (ActiveOrders[k].amount=2*StrToFloat(Form1.EditProcentCTbuy.Text)) then
             begin
                  begin
                   if not ((ActiveOrders[k].rate = (BidsPrice2+0.0005)) { or FoundedOneOrder}) then  //отменяем старые ордера с таким же объемом, но другой ценой
                    if Form1.BidsTimer2.Tag=0 then
                     begin
                      CancelOrder(ActiveOrders[k].order_id);
                     end;
                  end;
         ...
         FoundedSameOrder:= false;
         for k := 0 to Length(ActiveOrders) - 1 do      // ищем существующий ордер для стенки 2
          if ActiveOrders[k].pair = 'ltc_usd' then
            if ActiveOrders[k].OType = 'buy' then
             begin
              if ((BidsPrice2+0.0005) = (ActiveOrders[k].rate)) then
               begin
                FoundedSameOrder:=true;
               end;
             end;
        ...
        if not FoundedSameOrder then    //ставим ордер на стенку 2 если не обнаружен точно такой же ранее выставленный ордер
        if (Balance.USD/(BidsPrice2+0.0005))>=(2*StrToFloat(Form1.EditProcentCTbuy.Text)*1.001) then
        if Form1.BidsTimer2.Tag=0 then
         begin
           SendOrder('ltc_usd', 'buy', BidsPrice2+0.0005, 2*StrToFloat(Form1.EditProcentCTbuy.Text), true);
           Form1.BidsTimer2.Enabled:=True;  // ставим таймер на 3 секунды, что бы не выставить в цикле несколько ордеров подряд из-за позднего ответа биржи
           Form1.BidsTimer2.Tag:=1;
         end;
         //--------
        ...
        for k := 0 to Length(ActiveOrders) - 1 do
           if ActiveOrders[k].pair = 'ltc_usd' then
            if ActiveOrders[k].OType = 'buy' then
            if (ActiveOrders[k].amount=4*StrToFloat(Form1.EditProcentCTbuy.Text)) then
             begin
                  begin
                   if not ((ActiveOrders[k].rate = (BidsPrice3+0.0005)) { or FoundedOneOrder}) then  //отменяем старые ордера с таким же объемом, но другой ценой
                    if Form1.BidsTimer3.Tag=0 then
                     begin
                      CancelOrder(ActiveOrders[k].order_id);
                     end;
                  end;
         ...
         for k := 0 to Length(ActiveOrders) - 1 do      // ищем существующий ордер для стенки 3
          if ActiveOrders[k].pair = 'ltc_usd' then
            if ActiveOrders[k].OType = 'buy' then
             begin
              if ((BidsPrice3+0.0005) = (ActiveOrders[k].rate)) then
               begin
                FoundedSameOrder:=true;
               end;
             end;
        ...
        if not FoundedSameOrder then    //ставим ордер на стенку 3 если не обнаружен точно такой же ранее выставленный ордер
        if (Balance.USD/(BidsPrice3+0.0005))>=(4*StrToFloat(Form1.EditProcentCTbuy.Text)*1.001) then
        if Form1.BidsTimer3.Tag=0 then
         begin
           SendOrder('ltc_usd', 'buy', BidsPrice3+0.0005, 4*StrToFloat(Form1.EditProcentCTbuy.Text), true);
           Form1.BidsTimer3.Enabled:=True; // ставим таймер на 3 секунды, что бы не выставить в цикле несколько ордеров подряд из-за позднего ответа биржи
           Form1.BidsTimer3.Tag:=1;
         end;
         //--------
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            VirtualOrders[LengthVO].rate := TopPriceL + Strateg1[i].delta1F1;       // было минус
            VirtualOrders[LengthVO].min_rate := TopPriceL + Strateg1[i].delta1F1;   // было минус
          end
          else if Strateg1[i].SType = 1 then
          begin
            TopPriceL := TopPrice(Strateg1[i].pair, 'buy');
            VirtualOrders[LengthVO].OType := 'sell';
            VirtualOrders[LengthVO].rate := TopPriceL - Strateg1[i].delta1F1;       // было плюс
            VirtualOrders[LengthVO].min_rate := TopPriceL - Strateg1[i].delta1F1;   // было плюс
          end;
          ...
          for j := 0 to Length(VirtualOrders) - 1 do
            if (not VirtualOrders[j].deleted) and (VirtualOrders[j].pair = Strateg1[i].pair) and (VirtualOrders[j].comment = 'Strateg1('+IntToStr(i)+')') and
              ...
              begin
                ...
                NewRate := VirtualOrders[j].rate - VirtualOrders[j].stepF1;     //было VirtualOrders[j].stepF1
                if (NewRate >= TopPriceL + VirtualOrders[j].deltaF1) then      // + change -
                // Можно трейлить
                begin
                  Log('Передвинул Stage1 Ордер Buy c ' + FloatToStr(VirtualOrders[j].rate) + ' на ' + FloatToStr(NewRate));
...
begin
  ...
  begin
    ...
    begin
      ...
      Log(ResultTrail);
      if AnsiPos('Ispolnen order po Trailing Stop', ResultTrail) > 0 then
        begin
...
procedure TForm1.AskTimer2Timer(Sender: TObject);
begin
  Form1.AskTimer2.Enabled:=false;
  Form1.AskTimer2.Tag:=0;
end;
procedure TForm1.AskTimer3Timer(Sender: TObject);
begin
 Form1.AskTimer3.Enabled:=false;
  Form1.AskTimer3.Tag:=0;
end;
...
// Закрытие формы
procedure TForm1.AskTimer1Timer(Sender: TObject);
begin
  Form1.AskTimer1.Enabled:=false;
  Form1.AskTimer1.Tag:=0;
end;
procedure TForm1.BidsTimer1Timer(Sender: TObject);
begin
  Form1.BidsTimer1.Enabled:=false;
  Form1.BidsTimer1.Tag:=0;
end;

procedure TForm1.BidsTimer2Timer(Sender: TObject);
begin
  Form1.BidsTimer2.Enabled:=false;
  Form1.BidsTimer2.Tag:=0;
end;

procedure TForm1.BidsTimer3Timer(Sender: TObject);
begin
  Form1.BidsTimer3.Enabled:=false;
  Form1.BidsTimer3.Tag:=0;
end;
...
procedure delay(msec:cardinal);    // Добавил Наиль
var t:cardinal;
begin
 t:=GetTickCount;
...
function KillProcess(ExeName: string): LongBool;    // Добавил Наиль
var
 B: BOOL;
 ProcList: THandle;
 PE: TProcessEntry32;
...
procedure TForm1.BTC_Proxy_RunAndKillTimer(Sender: TObject);
begin
try
   Form1.UpdateTimer.Enabled:=false;
   Form1.StartThr.Enabled:=false;
   Form1.server.Active:=false;
   BTC_Proxy_RunAndKill.Tag:=1;
   Sleep(5500);
   KillProcess('btceproxy.exe'); // Добавил Наиль
   delay(3000);                  // Добавил Наиль
   KillProcess('btceproxy2.exe');
   delay(3000);
   WinExec('btceproxy.exe', 0);
   WinExec('btceproxy2.exe', 0);
   BTC_Proxy_RunAndKill.tag:=0;
   //Log('Перезагрузили btceproxy');
  finally
   Form1.UpdateTimer.Enabled:=true;
   Form1.StartThr.Enabled:=true;
